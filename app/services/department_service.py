...
class DepartmentService:
    ...
    async def create_department(
        ...
        existing = await self.department_repo.get_by_name_and_parent(
            name=data.name,
        ...
        department = await self.department_repo.create(data.name, data.parent_id)
    ...
    async def get_department_details(
        ...
        department = await self.department_repo.get_department_detail(department_id)
        if not department:
            raise DepartmentNotFoundError()

        return DepartmentTreeSchema.from_orm_tree(
            department,
            current_depth=0,
            max_depth=depth,
    ...
    async def update_department(
        ...
        if "parent_id" in update_data:
            ...
            if new_parent_id is not None:
                new_parent = await self.department_repo.get_department_detail(new_parent_id)

                if not new_parent:
                    raise ParentNotFoundError()

                children_ids = await self.department_repo.get_children_ids_recursive(department)
        ...
        if "name" in update_data:
            ...
            existing = (
                await self.department_repo.get_by_name_and_parent(
    ...
    async def delete_department(
        ...
        reassign_department = await self.department_repo.get_by_id(reassign_to_department_id)
